...
tools/lab-sweep/lab-table <lab-dir>                    # render the paper's tables
tools/lab-sweep/lab-table <lab-dir> --check            # stale? exit 1
```
...
| `tables` | tables a paper renders from the records — the next section |
...
## Tables: a number enters a paper only through a record

A study may declare `tables` (#209). Each names the varied parameter whose
levels become its rows (or `"cells"`, one row per configuration) and a list
of columns; a column pins the other varied parameters with `fix`, gives an
expression and a digit count:
...
"tables": {
  "pair": {
    ...
    "columns": [
      ...
      { "head": "parallel ÷ series", "expr": "col('parallel I (mA)') / col('series I (mA)')", "digits": 2 }
    ]
  }
}
```

`expr` is a deliberately small grammar: numbers, `+ - * /`, parentheses,
`<statistic>(<probe>)` for the seven statistics a record header carries
(`mean`, `stddev`, `min`, `max`, `first`, `last`, `count`), and `col('<head>')`
for an earlier column of the same row. Nothing else parses. Seeds are
aggregated per cell with `over`: `"mean"` (default), `"spread"` (max − min)
or `"n"`. Every rendered row ends with the ids of the records it was read
from, unless the table sets `"records": false`. Probes an expression reads
are recorded whether or not `record.probes` lists them, same rule as `report`.

`lab-table` renders a table between markers in the study's `study.md` or the
lab's `paper.md`:

```
<!-- table: series-vs-parallel/pair -->
...replaced by the rendering...
<!-- /table -->
```

`<study>/<name>` resolves against the lab's `studies/`; inside a `study.md`
the prefix may be dropped. A full sweep re-renders the study's own tables;
`lab-table <lab>` renders the paper's. `lab-table --check` is what
`tools/lab-check/lab-check` runs as its `tables` check: a block that differs
from the records — typed by hand, or left behind by a sweep whose numbers
moved — is a red line in a build.

The manifest is where a table is declared, not the marker, for the same
reason the manifest is inside the study: the one place a reviewer reads to
know what was run is also the one place that says what was tabulated. The
cost is that a paper table always belongs to a study; lab-level `records/`
cannot feed one.
...
expansion, the answerability check, record reading, the table grammar and
the rendering — so it needs no graphics session and never skips. The subprocess half is exercised by the
