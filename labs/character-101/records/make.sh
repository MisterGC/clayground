...
#
# Produces the committed run records for character-101: one stepped run per
# aspect, in the state the aspect cold-opens into. A sheet's probes read the
# pure pose model at the phase the clock stands in, so a record of the gait
# sheet is the cycle's own curves; the scenes whose motion belongs to the
# plugin's own animators (speech, conversation, crowd) record what a stepped
# run honestly sees of them, which is nothing moving.
#
#     labs/character-101/records/make.sh            # every scenario
#     labs/character-101/records/make.sh gait       # one of them
#     labs/character-101/records/make.sh --verify   # determinism check
#     labs/character-101/records/make.sh --out-dir DIR   # regenerate elsewhere
#
...
SEED=42
STEPS=600           # 10 simulated seconds at 1/60 s, sampled every 0.05 s
SCENARIOS=(lineup gait gestures action moves hands faces heads speech conversation crowd)
