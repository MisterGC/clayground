...
// The transistor as the circuit kit's own anatomy: at spread 0 this IS
// CircuitElement3D's transistor - the same epoxy cylinder at (0, 1.55, -0.35),
// the same flat facet, the same three legs lying on the board half-way out to
// the same pads, the same printed footprint with C / B / E. The element loads
// this file for its body, so there is no "the real part" and "the anatomy" to
// keep in step: there is one part, which can come apart.
//
// Only the working-region collar stays with the element: it says what the part
// is DOING, which an anatomy is not about, and it has no place in a lesson that
// takes the package off.
//
// Everything above the board that the assembled part does not show is the
// inside: the metal header on the collector lead, the three-layer die from
...
//
// Every part's id, role, teaching order, stage, travel and mark anchor come
// from ONE row of anatomy.js. Nothing here is hand-placed except the assembled
// pose, which is geometry and belongs beside the models it has to match.
//
// The two admitted colour literals are the epoxy and the facet: physical part
// colours, the same values the kit pins for those two surfaces, not theme
// roles. Everything else comes from the role table.
ExplodedView3D {
    id: root
    ...
    // Physical part colours, not theme roles - the values CircuitElement3D
    // pins for the same two surfaces.
    readonly property color epoxyColor: "#2a2724"
    readonly property color facetColor: "#332f2b"

    /*! The role-to-colour table: what a part is MADE OF, as one switch. */
    ...
    // Flat, glare-free material for the round parts - the kit's own.
    ...
    // Toon-shaded box with ink edges - the kit's boxes. Origin is the box's
    // BOTTOM-centre, which is why every slab is positioned at its own y0.
    ...
    // which leg is which. Opaque edge to edge, because this material does no
    // alpha blending and a cut corner comes out BLACK; any shaping has to be
    // painted.
    ExplodePart {
        id: _print
        row: Anatomy.partById("print")
    ...
    // board, exactly as the kit lays them. A #Cylinder stands along Y, so the two
    // side legs tip onto X and the base leg onto Z. The part's origin is mid-leg,
    // which is also where a fingertip belongs.
    ExplodePart {
        id: _legC
        row: Anatomy.partById("leg.collector")
    ...
    ExplodePart {
        id: _legE
        row: Anatomy.partById("leg.emitter")
    ...
    // ghost with the x-ray; both lift clear of the inside in stage 1.
    ExplodePart {
        id: _case
        row: Anatomy.partById("case")
    ...
    // emitter is a small island on top. Each slab's anchor is its top face (the
    // table says so), so a ring on "the base" sits on the surface a learner can
    // see rather than buried in the layer below.
    ExplodePart {
        id: _dieC
        row: Anatomy.partById("die.collector")
    ...
    ExplodePart {
        id: _dieE
        row: Anatomy.partById("die.emitter")
    ...
    // The x-ray interpolant, same shape as the base type's spread glide.
    QtObject {
        id: _xglide
