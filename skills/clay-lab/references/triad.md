...
The first two are mechanical (`lab-sweep --check` proves them). The third
is the one that matters: only the model card can say whether the physics
holds for *this* question, which is why every kit has one and states its
simplifications with a direction of effect. Whatever stays unmapped forces
one of exactly two outcomes, stated before the method: a **reduced
question** the lab genuinely can hold, or **"not answerable with this
lab"** with the missing quantity named. Neither is a failure; confident
numbers about a question the model cannot hold are. When a knob is
missing, add it (and say the lab changed) or reduce the question — never
approximate it with something adjacent unmentioned.

#### A study — one question, in one place

A lab is a capability space; a **study** is one question posed against it:
`labs/<lab>/studies/<slug>/` with `study.md`, committed `records/`, a
`results.md` that `lab-sweep` generates and nobody hand-edits, and an
optional `figures/` with its own `make.sh`. `tools/lab-sweep/README.md`
documents the manifest and the runner;
`labs/street-network-101/studies/topology-four-houses/` is the reference.

`study.md` is written **in this order, and the order is the point** — the
validity argument is made before any number exists, so it cannot be
written to fit the answer: the question (with what "better" means), the
answerability mapping with what had to be reduced or added, the method and
the manifest, results quoted by record id, the conclusion with its limits.
...
everything above it without `records/` and `results.md`.
...
- **The recording is stepped, by a committed driver.** `records/make.sh`
  (generated by `tools/lab-new`, sensor-fusion-101 has the reference) drives
  `clayrender --paused`, builds each record's `command` field out of its own
  path, and `--verify` runs each scenario twice and `cmp`s the files — that
  command *is* the determinism evidence.
...
labs/<lab>/figures/            beside paper.md
    ...
    <name>.png                 committed, never edited by hand
```
...
1. **Every figure is regenerated by `make.sh`, or it is not a figure.** A
   picture nobody can remake is a screenshot: it rots the first time the lab
   changes and nothing tells you. "Does the paper still look like the lab?"
   is a re-run and a `git diff`.
2. **Photograph a chosen instant.** `--paused`, step the clock to the moment
   worth showing, `--settle` so the capture waits for the frame to stop
   changing. A lab left to play itself gives a different picture every run.
3. **Crop by name.** `--crop <objectName> --crop-pad 8`; give anything a
   figure might want an `objectName` (the kernel's one-per-lab widgets have
   one: `belt`, `hint`). Never shrink the window and never downscale — both
   in `pitfalls.md`.
4. **A figure is illustrative; no number in the prose may be read off one.**
   Say so where the figures appear: *"the figures are illustrative only;
   every number comes from `records/`."* A caption may name a value a record
   backs; it may never be the source of one.
5. **Renders never inherit your settings** — every figure comes out at the
   default theme, language and UI scale, so a German figure needs
   `--eval 'LabLang.lang = "de"'` (`docs/docs/manual/clayrender.md`).

What earns one: the setup at the moment its lesson is visible; a close-up of
a reading so an instrument is recognisable when the text names it; a *pair*
on one seed where the argument is a contrast; a piece of UI the prose makes
a claim about. Decoration earns nothing. Reference:
`labs/electronics-101/figures/make.sh`.
