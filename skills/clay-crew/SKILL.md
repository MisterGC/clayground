...
- `phase: load_error` → the reload broke the scene. `errors` is the
  direct question; `snapshot` also still works, and even with no root
  item its response carries `errors`, `warnings`, and `logTail`. The last
  `phase_change` event also carries `errorsTail`.
- Change seems to have had no effect → check `status.generation` moved.
  If it did not, you are measuring the pre-edit scene.
...
2. **Load check:** `{"id": "...", "action": "waitForRoot", "timeoutMs": 5000}`
   ...
   phase is already terminal); response carries `phase`, and `status`
   tells you whether `generation` advanced. On `load_error`, fix the
   errors (`errors` action) before anything else.
...
state survives). The response acknowledges with `reloadStatus:
"requested"` — the reload has not happened yet. Follow with `waitForRoot`,
then confirm `status.generation` advanced. With `scenario`, the root's
